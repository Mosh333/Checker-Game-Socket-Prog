import tkinter as tk
class GameBoard(tk.Frame):
    def __init__(self, parent, rows=8, columns=8, size=40, color1="white", color2="grey"):
        '''size is the size of a square, in pixels'''

        self.rows = rows
        self.columns = columns
        self.size = size
        self.color1 = color1
        self.color2 = color2
        self.pieces = {}

        canvas_width = columns * size
        canvas_height = rows * size

        tk.Frame.__init__(self, parent)
        self.canvas = tk.Canvas(self, borderwidth=0, highlightthickness=0,
                                width=canvas_width, height=canvas_height, background="goldenrod3")
        self.canvas.pack(side="top", fill="both", expand=True, padx=2, pady=2)


        # this binding will cause a refresh if the user interactively
        # changes the window size
        self.canvas.bind("<Configure>", self.refresh)

    def addpiece(self, name, image, row=0, column=0):
        '''Add a piece to the playing board'''
        self.canvas.create_image(0,0, image=image, tags=(name, "piece"), anchor="c")
        self.placepiece(name, row, column)

    def placepiece(self, name, row, column):
        '''Place a piece at the given row/column'''
        self.pieces[name] = (row, column)
        x0 = (column * self.size) + int(self.size/2)
        y0 = (row * self.size) + int(self.size/2)
        self.canvas.coords(name, x0, y0)

    def refresh(self, event):
        '''Redraw the board, possibly in response to window being resized'''
        xsize = int((event.width-1) / self.columns)
        ysize = int((event.height-1) / self.rows)
        self.size = min(xsize, ysize)
        self.canvas.delete("square")
        color = self.color2
        for row in range(self.rows):
            color = self.color1 if color == self.color2 else self.color2
            for col in range(self.columns):
                x1 = (col * self.size)
                y1 = (row * self.size)
                x2 = x1 + self.size
                y2 = y1 + self.size
                self.canvas.create_rectangle(x1, y1, x2, y2, outline="black", fill=color, tags="square")
                color = self.color1 if color == self.color2 else self.color2
        for name in self.pieces:
            self.placepiece(name, self.pieces[name][0], self.pieces[name][1])
        self.canvas.tag_raise("piece")
        self.canvas.tag_lower("square")


# image comes from the silk icon set which is under a Creative Commons
# license. For more information see http://www.famfamfam.com/lab/icons/silk/
imagedata = '''
    
    yxYYEAA7
'''



if __name__ == "__main__":
    root = tk.Tk()
    root.title("Checkers")
    #root.resizeable(width=False, height=False)
    root.minsize(width=640, height=320)
    root.maxsize(width=640, height=320)
    board = GameBoard(root)
    board.pack(side="top", fill="both", expand="true", padx=4, pady=4)
##        myPlayer = {}
##    for i in range(64):
##        myPlayer['player%02d' % i] = tk.PhotoImage(data=imagedata)
##    for row in range(8):
##        for col in range(8):
##            board.addpiece("player", myPlayer[i], 0,0)
##    player1 = tk.PhotoImage(data=imagedata)
##    board.addpiece("player1", player1, 0,0)
##    player2 = tk.PhotoImage(data=imagedata)
##    board.addpiece("player2", player2, 7,7)
##    player3 = tk.PhotoImage(data=imagedata)
##    board.addpiece("player3", player2, 3,3)
    blackking = tk.PhotoImage(file="blackking.gif")
    blackpiece = tk.PhotoImage(file="blackpiece.gif")
    redking = tk.PhotoImage(file="redking.gif")
    redpiece = tk.PhotoImage(file="redpiece.gif")
    
    player1 = tk.PhotoImage(file="blackpiece.gif")
    board.addpiece("player1", player1, 0,0)
    player2 = tk.PhotoImage(file="blackpiece.gif")
    board.addpiece("player2", player2, 7,7)
    player3 = tk.PhotoImage(file="blackpiece.gif")
    board.addpiece("player3", player2, 3,3)
    player4 = tk.PhotoImage(file="blackpiece.gif")
    board.addpiece("player4", player1, 2,2)
    player5 = tk.PhotoImage(file="blackpiece.gif")
    board.addpiece("player5", player1, 7,1)
    root.mainloop()
